...
namespace FATE.FATECombat.Runtime.Utility
{
    public static class CombatCalculations
    {
        ...
        private static void HandleStatEffects(CombatEntity casterEntity, CombatEntity targetEntity, RPGAbilityRankData abilityRank, RPGEffect.RPGEffectRankData effectRank, DamageResult result)
        {
            if (casterEntity == null) return;
            if (abilityRank == null || effectRank.alteredStatID != GameDatabase.Instance.GetCombatSettings().HealthStatID) return;
            if (!CombatUtilities.AbilityHasTag(abilityRank, AbilityTags.OnHit)) return;
                ...
                foreach (var t1 in t.Value.stat.statBonuses)
                {
                    switch (t1.statType)
                    {
                        case RPGStat.STAT_TYPE.EFFECT_TRIGGER:
                            ...
                            foreach (var t2 in t.Value.stat.onHitEffectsData.Where(t2 => t2.tagType == AbilityTags.OnHit).Where(t2 => Random.Range(0f, 100f) <= t2.chance))
                            {
        ...
        public static DamageResult DamageCalculation(CombatEntity casterEntity, CombatEntity targetEntity, RPGAbilityRankData abilityRank, RPGEffect effect, RPGEffect.RPGEffectRankData effectRank, bool isDamageOverTime)
        {
